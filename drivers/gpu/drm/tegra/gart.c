...
MODULE_PARM_DESC(gart_security_level,
	"Memory protection level (0 = light, 1 = enhanced, 2 = best effort, 3 = strict)");
			    ...
			    size_t size)
{
	...
	drm_mm_scan_init(&scan, &tegra->mm, size, 1UL << order, 0, insert_mode);
				    ...
				    bool enospc_fatal)
{
	 ...
	 * GART's top.
	 */
	if (bo->gem.size < SZ_512K)
		insert_mode = DRM_MM_INSERT_HIGH;
	else
		insert_mode = DRM_MM_INSERT_BEST;
...
mm_ok:
	...
	if (!bo->iosize) {
		...
		WARN_ON_ONCE(bo->gem.size < bo->iosize);
	}
